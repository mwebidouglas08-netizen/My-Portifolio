...
/* ── Security & Middleware ── */
app.use(helmet({
  contentSecurityPolicy: false, // Disabled to allow inline SVG/styles in vanilla setup
...
app.post('/api/contact', contactLimiter, function (req, res) {
  ...
  const hasRealCreds = EMAIL_USER.length > 0 &&
    EMAIL_PASS.length >= 16 &&
    !EMAIL_PASS.includes('your_') &&
    !EMAIL_PASS.includes('PASTE') &&
  ...
  const ownerHtml = [
    ...
    `<tr><td style="padding:8px 0;border-bottom:1px solid #1a1a24;font-family:monospace;font-size:11px;color:#50506a">EMAIL</td><td style="padding:8px 0;border-bottom:1px solid #1a1a24"><a href="mailto:${esc(email)}" style="color:#5eead4">${esc(email)}</a></td></tr>`,
...
app.get('/health', function (_req, res) {
  ...
  const configured = p.length >= 16 && !p.includes('your_') && !p.includes('PASTE');
  res.json({
    status: 'ok',
...
app.listen(PORT, function () {
  const p = process.env.EMAIL_PASS || '';
  const ok = p.length >= 16 && !p.includes('your_') && !p.includes('PASTE');
  console.log(`\n🚀  Daggy Portfolio running on http://localhost:${PORT}`);
  console.log(`📧  Email configured: ${ok}`);
  if (!ok) {
    console.log('\n  To enable email, set these in .env or Render dashboard:');
